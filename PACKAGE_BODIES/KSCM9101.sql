--------------------------------------------------------
--  DDL for Package Body KSCM9101
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PREMIER"."KSCM9101" 
IS
  PROCEDURE control
  (
  ...
  <div id="top-header" style="padding-bottom: 2px;background-color: #ffffff;">
    ...
    <div id="c00">
      ...
      <div id="c11">
        ...
        <a href="#" target="list" onclick="doQuery(); return false;" class="btn-search buttons">'||get_fn('선택조회')||'</a>
      </div>
      <div class="crlf"></div>
    </div>
    <div id="c22">
      ...
      <title>'||common.title||'</title>
        ...
        <script type="text/javascript">
          ...
          function doInitGrid(){
            ...
            x[i]=["'||get_fn('생산/납기일')||'", "*","center","ro","str","true"];i++;
  ...
  IS
    cursor cur is
      select '20221111' as gubun from dual
      union
      select '20221112' as gubun from dual
      union
      select '20221120' as gubun from dual
      ;
...
--      v_link := '^javascript:link_(\"'||rec(i).companycd||'\", \"'||rec(i).vendcd||'\", \"'||rec(i).partno||'\", \"'||rec(i).sno||'\")^_self';
      ...
      <title>'||common.title||'</title>
        ...
        <script type="text/javascript">
          ...
          function doInitGrid(){
            ...
            x[i]=["'||get_fn('수량')||'", "60","left","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            x[i]=["'||get_fn('변경수량')||'", "60","left","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            x[i]=["'||get_fn('변경일자')||'", "80","left","ro","str","true"];i++;
            x[i]=["'||get_fn('발주번호')||'", "150","left","ro","str","true"];i++;
            x[i]=["'||get_fn('요청거래처')||'", "150","left","ro","str","true"];i++;
  ...
  IS
    cursor cur is
      select '20221123' as kindnm
            ,'4270-2335' as partno
            ,'REG ASSY-PR LH' as partnm
            ,'3560' as qty
            ,'5500' as amendqty
            ,'2022/11/11' as moddate
            ,'5551' as ordno
            ,'kwangjin' as vendnm
      from dual
      ;
