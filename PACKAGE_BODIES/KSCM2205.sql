--------------------------------------------------------
--  DDL for Package Body KSCM2205
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PREMIER"."KSCM2205" 
IS
  PROCEDURE control
  (
  ...
  <div id="top-header" style="padding-bottom: 2px;background-color: #ffffff;">
    ...
    <div id="c00">
      ...
      <div class="cBox">
        <div class="cTitle"><label for="p_symd">'||get_fn('기준일자')||'</label></div>
        <div class="cInput">
      ...
      <title>'||common.title||'</title>
        ...
        <script type="text/javascript">
          ...
          function doInitGrid(){
            ...
            x[i]=["'||get_fn('품번')||'", "160","left","ro","str","true"];i++;
            x[i]=["'||get_fn('품명')||'", "200","left","ro","str","true"];i++;
            x[i]=["'||get_fn('기초재고')||'", "100","left","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            x[i]=["'||get_fn('입고(사급)')||'", "100","left","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            x[i]=["'||get_fn('출고')||'", "100","left","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            x[i]=["'||get_fn('이론재고')||'", "100","left","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            x[i]=["'||get_fn('기말재고')||'", "100","left","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            x[i]=["'||get_fn('차이')||'", "100","left","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
  ...
  IS
    cursor cur is
      select '91070-28335' as partno
            ,'TEG4 ASSY-PR LH' as partnm
            ,'99645' as basicqty
            ,'13411' as inqty
            ,'274821' as outqty
            ,'170181' as thinkqty
...
--      v_link := '^javascript:link_(\"'||rec(i).companycd||'\", \"'||rec(i).vendcd||'\", \"'||rec(i).partno||'\", \"'||rec(i).sno||'\")^_self';
      ...
      { id:' ||i|| ',
        data:[
          ...
          {value:"'||rec(i).partno||'"},
          {value:"'||rec(i).partnm||'"},
          {value:"'||rec(i).basicqty||'"},
          {value:"'||rec(i).inqty||'"},
          {value:"'||rec(i).outqty||'"},
          {value:"'||rec(i).thinkqty||'"},
          {value:"'||rec(i).lastqty||'"},
          {value:"'||rec(i).chai||'"}
        ]
      } ');
