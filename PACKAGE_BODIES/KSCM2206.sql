--------------------------------------------------------
--  DDL for Package Body KSCM2206
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PREMIER"."KSCM2206" 
IS
  PROCEDURE control
  (
      ...
      <title>'||common.title||'</title>
        ...
        <script type="text/javascript">
          ...
          function doInitGrid(){
            ...
            x[i]=["'||get_fn('품번')||'", "130","center","ro","str","true"];i++;
            x[i]=["'||get_fn('품명')||'", "260","left","ro","str","true"];i++;
            x[i]=["'||get_fn('차종')||'", "100","center","ro","str","true"];i++;
            x[i]=["'||get_fn('투입중량(kg)')||'", "100","center","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            x[i]=["'||get_fn('제품중량(kg)')||'", "100","center","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            x[i]=["'||get_fn('SR중량(kg)')||'", "100","center","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            x[i]=["'||get_fn('cavity')||'", "100","center","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            x[i]=["'||get_fn('Loss율(%)')||'", "100","center","ro","str","true"];i++;
            x[i]=["'||get_fn('불량율(%)')||'", "100","center","ro","str","true"];i++;
  ...
  IS
    ...
    cursor cur is
      select '43270-2335' as partno
            ,'DEG4 ASSY-PR LH' as partnm
            ,'k3' as kindcd
            ,'115489' as t_weight
            ,'7488922' as weight
            ,'1212154' as sr_wgt
...
--      v_link := '^javascript:link_(\"'||rec(i).companycd||'\", \"'||rec(i).vendcd||'\", \"'||rec(i).partno||'\", \"'||rec(i).sno||'\")^_self';
      ...
      { id:' ||i|| ',
        data:[
          ...
          {value:"'||rec(i).partnm||'"},
          {value:"'||rec(i).kindcd||'"},
          {value:"'||rec(i).t_weight||'"},
          {value:"'||rec(i).weight||'"},
          {value:"'||rec(i).sr_wgt||'"},
          {value:"'||rec(i).cavity||'"},
          {value:"'||rec(i).loss||'"},
          {value:"'||rec(i).defect_yul||'"}
        ]
      } ');
