--------------------------------------------------------
--  DDL for Package Body KB2B1108
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PREMIER"."KB2B1108" 
IS
  PROCEDURE control
  (
  ...
  <div id="top-header" style="padding-bottom: 2px;background-color: #ffffff;">
    ...
    <div id="c00">
      ...
      <div class="cBox">
        <div class="cTitle"><label for="p_symd">'||get_fn('일자')||'</label></div>
        <div class="cInput">
      ...
      <title>'||common.title||'</title>
        ...
        <script type="text/javascript">
          ...
          function doInitGrid(){
            ...
            x[i]=["'||get_fn('포장완료일')||'", "70","center","ro","str","true"];i++;
            x[i]=["'||get_fn('컨테이너 번호')||'", "*","center","ro","str","true"];i++;
  ...
  IS
    cursor cur is
      select '20221110' as popo, '내부' as gubun  from dual
      union
      select '20221113' as popo, '협력사' as gubun from dual
      union
      select '20221115' as popo, '내부' as gubun from dual
      ;
...
--      v_link := '^javascript:link_(\"'||rec(i).companycd||'\", \"'||rec(i).vendcd||'\", \"'||rec(i).partno||'\", \"'||rec(i).sno||'\")^_self';
      ...
      <title>'||common.title||'</title>
        ...
        <script type="text/javascript">
          ...
          function doInitGrid(){
            ...
            x[i]=["NO", "40","center","ro","int","true"];i++;
            x[i]=["'||get_fn('포장완료일')||'", "100","center","ro","str","true"];i++;
            x[i]=["'||get_fn('컨테이너 번호')||'", "100","center","ro","str","true"];i++;
            x[i]=["'||get_fn('순번')||'", "100","center","ro","str","true"];i++;
            x[i]=["'||get_fn('품번')||'", "100","center","ro","str","true"];i++;
            x[i]=["'||get_fn('수량')||'", "100","center","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            x[i]=["'||get_fn('거래처')||'", "100","center","ro","str","true"];i++;
            x[i]=["'||get_fn('사업장')||'", "100","center","ro","str","true"];i++;
            x[i]=["'||get_fn('통화코드')||'", "100","center","ro","str","true"];i++;
  ...
  IS
    cursor cur is
      select '20221127' as popo
            ,'43458-44315' as conno
            ,'4' as sno
            ,'k5' as partno
            ,'23366' as qty
            ,'beach' as center
