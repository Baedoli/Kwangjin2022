--------------------------------------------------------
--  DDL for Package Body KSCM2104
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PREMIER"."KSCM2104" 
IS
  PROCEDURE control
  (
      ...
      <title>'||common.title||'</title>
        ...
        <script type="text/javascript">
          ...
          function doInitGrid(){
            ...
            x[i]=["'||get_fn('당일')||'", "50","left","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;

            '); for i in 1..13 loop htp.p('
            x[i]=["'||get_fn('+'||i)||'", "50","center","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;     
            '); end loop;  htp.p('
            
            x[i]=["'||get_fn('이론재고')||'", "70","right","ron","int","true"];mygrid.setNumberFormat("0,000", i);;i++;
