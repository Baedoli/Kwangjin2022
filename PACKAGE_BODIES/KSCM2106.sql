--------------------------------------------------------
--  DDL for Package Body KSCM2106
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PREMIER"."KSCM2106" 
IS
  PROCEDURE control
  (
  ...
  <div id="top-header" style="padding-bottom: 2px;background-color: #ffffff;">
    ...
    <div id="c00">
      <div id="c01">
        ...
        <div class="cBox">
          <div class="cTitle"><label for="p_yymm">'||get_fn('기준년월')||'</label></div>
          <div class="cInput">
            <input type="text" id="p_yymm" name="p_yymm" maxlength="6" value="'||p_yymm||'" class="cDate">
          </div>
        </div>
      ...
      <title>'||common.title||'</title>
        ...
        <script type="text/javascript">
          ...
          function doInitGrid(){
            ...
            x[i]=["'||get_fn('업체명')||'", "140","left","ro","str","true"];i++;
            x[i]=["'||get_fn('사업장')||'", "100","center","ro","str","true"];i++;
            x[i]=["'||get_fn('차종')||'", "100","center","ro","str","true"];i++;
            x[i]=["'||get_fn('품번')||'", "100","center","ro","str","true"];i++;
            x[i]=["'||get_fn('품명')||'", "140","left","ro","str","true"];i++;
            
            '); for i in 1..11 loop htp.p('
            x[i]=["'||to_char(add_months(to_date(p_yymm, 'YYYYMM'), i), 'yyyy.mm')||'", "100","center","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            '); end loop; htp.p('
