--------------------------------------------------------
--  DDL for Package Body KSCM6105
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PREMIER"."KSCM6105" 
IS
  PROCEDURE control
  (
  ...
  IS
    cursor cur is
      select 'linename88' as gubun from dual
      union
      select 'linename35' as gubun from dual
      union
      select 'linename67' as gubun from dual
      ;
      ...
      <title>'||common.title||'</title>
        ...
        <script type="text/javascript">
          ...
          function doInitGrid(){
            ...
            x[i]=["'||get_fn('대표품번')||'", "150","center","ro","str","true"];i++;
            x[i]=["'||get_fn('품명')||'", "*","left","ro","str","true"];i++;
            x[i]=["'||get_fn('기초재고')||'", "100","right","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            x[i]=["'||get_fn('실적수량')||'", "100","right","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            x[i]=["'||get_fn('출고')||'", "100","right","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            x[i]=["'||get_fn('이론재고')||'", "100","right","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
...
--      v_link := '^javascript:link_(\"'||rec(i).companycd||'\", \"'||rec(i).vendcd||'\", \"'||rec(i).partno||'\", \"'||rec(i).sno||'\")^_self';
      ...
      { id:' ||i|| ',
        data:[
          {value:"'||i||'"},
          {value:"'||rec(i).gubun||'"},
          {value:"'||rec(i).bestpart||'"},
          {value:"'||rec(i).partnm||'"},
          {value:"'||rec(i).qty||'"},
          {value:"'||rec(i).amendqty||'"},
          {value:"'||rec(i).amendqty2||'"},
          {value:"'||rec(i).gubun2||'"}
        ]
      } ');
