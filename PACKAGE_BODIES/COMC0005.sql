--------------------------------------------------------
--  DDL for Package Body COMC0005
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PREMIER"."COMC0005" 
IS
  PROCEDURE control
  (
  ...
  <head>
    ...
    <script>
      ...
      function doCrrec() {  
        var url = "'||v_pkg||'.crrec";  
        newWnd2 = window.open(url,"'||v_pkg||'window","top=100, left=100, status=yes, resizable=no, scrollbars=yes, width=800, height=193");  
        newWnd2.focus();
      } 
      ...
      <title>'||common.title||'</title>
        ...
        <script type="text/javascript">
          ...
          function doInitGrid(){
            ...
            x[i]=["'||get_fn('시스템코드')||'", "100","center","link","str","true"];i++;
            x[i]=["'||get_fn('시스템명')||'", "500","left","ro","str","true"];i++;
            x[i]=["'||get_fn('사용유무')||'", "100","center","ro","str","true"];i++;
          ...
          function doOnRowSelected(rowID,celInd){
            ...
            var v_url_b = "'||v_pkg||'.query_b?p_systemcd="+p_systemcd;
            parent.myLayout.cells("b").attachURL(v_url_b);
          }
          
          function link_(obj1){
            url ="'||v_pkg||'.crrec?p_systemcd="+obj1;
            newWnd2 = window.open(url,"'||v_pkg||'window","top=100, left=100, status=yes, resizable=no, scrollbars=yes, width=800, height=193");  
            newWnd2.focus();
          }
  ...
  IS
    ...
    cursor cur is
      select  systemcd, systemnm, useyn
      from    c_0005
  ...
  BEGIN
    ...
    for i in 1 .. v_number loop
      v_link := '^javascript:link_(\"'||rec(i).systemcd||'\")^_self';
      htp .p('
      { id:' ||i|| ',
        data:[
          {value:"'||i||'"},
          {value:"'||rec(i).systemcd||v_link||'"},
          {value:"'||rec(i).systemnm||'"},
          {value:"'||rec(i).useyn||'"}
        ]
      } ');
          ...
          <div>
            <table width="100%" border="0" cellspacing="0" cellpadding="4">
              <tbody>
                ...
                <tr>
                  <td class="titletd require">'||get_fn('시스템코드')||'</td>
                  <td class="inputtd">
                    <input type="text" id="p_systemcd" name="p_systemcd" maxlength="3" '||v_disabled||' value="'||rec.systemcd||'" /> 
                  </td>
                  <td class="titletd">'||get_fn('시스템명')||'</td>
                  <td class="inputtd">
                    <input type="text" id="p_systemnm" name="p_systemnm" value="'||rec.systemnm||'" / > 
                  </td>
                </tr>
                <tr>
  ...
  BEGIN
    ...
    if p_dml_gubun = '1' then
      ...
      begin
        insert into c_0005
        (systemcd, systemnm, useyn)
        values
        (p_systemcd, p_systemnm, p_useyn);
      exception
        when dup_val_on_index then
          update  c_0005
          set     systemnm = p_systemnm, 
                  useyn = p_useyn
          where   1=1
          and     systemcd = p_systemcd
          ;
      ...
      <script type="text/javascript">
        ...
        location.href = "'||v_pkg||'.crrec?p_systemcd='||p_systemcd||'";
      </script>
      ');
