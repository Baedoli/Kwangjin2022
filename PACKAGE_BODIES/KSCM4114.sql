--------------------------------------------------------
--  DDL for Package Body KSCM4114
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PREMIER"."KSCM4114" 
IS
  PROCEDURE control
  (
  ...
  <div id="top-header" style="padding-bottom: 2px;background-color: #ffffff;">
    ...
    <div id="c00">
      ...
      <div class="cBox">
        <div class="cTitle"><label for="p_symd">'||get_fn('생산납기일')||'</label></div>
        <div class="cInput">
      ...
      <title>'||common.title||'</title>
        ...
        <script type="text/javascript">
          ...
          function doInitGrid(){
            ...
            x[i]=["'||get_fn('생산납기일')||'", "*","center","ro","str","true"];i++;
  ...
  IS
    cursor cur is
      select '22211-15448' as gubun from dual
      union
      select '22212-15448' as gubun from dual
      union
      select '22213-15448' as gubun from dual
      ;
...
--      v_link := '^javascript:link_(\"'||rec(i).companycd||'\", \"'||rec(i).vendcd||'\", \"'||rec(i).partno||'\", \"'||rec(i).sno||'\")^_self';
      ...
      <title>'||common.title||'</title>
        ...
        <script type="text/javascript">
          ...
          function doInitGrid(){
            ...
            x[i]=["'||get_fn('품번')||'", "100","center","ro","str","true"];i++;
            x[i]=["'||get_fn('품명')||'", "150","center","ro","str","true"];i++;
            x[i]=["'||get_fn('발주량')||'", "80","center","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            x[i]=["'||get_fn('가입고대기')||'", "80","center","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            x[i]=["'||get_fn('검수대기')||'", "80","center","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            x[i]=["'||get_fn('입고수량')||'", "80","center","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            x[i]=["'||get_fn('미납량')||'", "80","center","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            x[i]=["'||get_fn('발주번호')||'", "250","center","ro","str","true"];i++;
  ...
  IS
    cursor cur is
      select '2' as gubun
            ,'13458-44315' as partno
            ,'cruze' as partnm
            ,'1366' as qty
            ,'2010' as a_qty
            ,'4311' as b_qty
            ,'3010' as s_qty
            ,'2233' as incomplete_qty
            ,'6' as ordno
      from dual
      ;
