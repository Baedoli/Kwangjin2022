--------------------------------------------------------
--  DDL for Package Body KSCM2203
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PREMIER"."KSCM2203" 
IS
  PROCEDURE control
  (
      ...
      <title>'||common.title||'</title>
        ...
        <script type="text/javascript">
          ...
          function doInitGrid(){
            ...
            x[i]=["'||get_fn('사업구분')||'", "100","left","ro","str","true"];i++;
            x[i]=["'||get_fn('판매품번')||'", "130","left","ro","str","true"];i++;
            x[i]=["'||get_fn('자재품번')||'", "130","left","ro","str","true"];i++;
            x[i]=["'||get_fn('원자재품번')||'", "130","left","ro","str","true"];i++;
            x[i]=["'||get_fn('원자재품명')||'", "180","left","ro","str","true"];i++;
            x[i]=["'||get_fn('중량(kg)')||'", "180","left","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
