--------------------------------------------------------
--  DDL for Package Body KSCM4111
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PREMIER"."KSCM4111" 
IS
  PROCEDURE control
  (
      ...
      <title>'||common.title||'</title>
        ...
        <script type="text/javascript">
          ...
          function doInitGrid(){
            ...
            x[i]=["'||get_fn('구분')||'", "100","center","ro","str","true"];i++;
            x[i]=["'||get_fn('품번')||'", "100","left","ro","str","true"];i++;
            x[i]=["'||get_fn('품명')||'", "*","left","ro","str","true"];i++;
            x[i]=["'||get_fn('생산납기일')||'", "100","left","ro","str","true"];i++;
            x[i]=["'||get_fn('수량')||'", "100","right","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            x[i]=["'||get_fn('납품량')||'", "100","right","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
  ...
  IS
    cursor cur is
      select '1' as gubun
            ,'43270-2335' as partno
            ,'REG4 ASSY-PR LH' as partnm
            ,'22/11/14' as nadate
            ,'3411' as qty
            ,'27481' as f_qty
      from dual
