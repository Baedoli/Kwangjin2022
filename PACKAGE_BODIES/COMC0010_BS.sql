--------------------------------------------------------
--  DDL for Package Body COMC0010_BS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PREMIER"."COMC0010_BS" 
IS
  PROCEDURE control
  (
  ...
  <div id="top-header" style="padding-bottom: 2px;background-color: #ffffff;">
    ...
    <div id="c22">
      <div class="floatL">
          <label for="p_companycd"></label>
          <select name="p_companycd" id="p_companycd" onchange="p_companycd_onchange(); return false;"> ');
            common.list_company(p_companycd, v_lang); htp.p('
          </select>      
          <label for="p_teamcd"> '||get_fn('부서')||'</label>
          
            '); common.list_dept(p_companycd, p_teamcd, 'select', null, 'p_teamcd', 'b', 'yes'); htp.p(' 
  ...
  <head>
    ...
    <script type="text/javascript">   
            ...
            for i in 1 .. rec2.count loop  
              ...
              htp.p('
                { id:'||i||',
                  data:[
                    {value:"'||i||'"},
                    {value:"'||return_html_text(rec2(i).companycd)||v_link2||'"},
                    ...
                    {value:"'||return_html_text(rec2(i).jikup)||'"},
                    {value:"'||return_html_text(rec2(i).position)||'"},
      ...
      function doInitGrid2(){
        ...
        x[i]=["NO", "40","center","ro","int","true"];i++;
        x[i]=["'||get_fn('사업장')||'", "80","center","link","str","true"];i++;
        x[i]=["'||get_fn('부서')||'", "80","center","ro","str","true"];i++;
        x[i]=["'||get_fn('기본적용여부')||'", "80","center","ro","str","true"];i++;
        x[i]=["'||get_fn('사용여부')||'", "80","center","ro","str","true"];i++;
              ...
              <tr>
                ...
                <td>
                  <select name="p_companycd" id="p_companycd" onchange="p_companycd_onchange();" '||v_disabled||'>
                    '); common.list_company(nvl(rec.companycd, p_companycd), v_lang); htp.p('
                  </select>
                </td>
                <td class="titletd require">'||get_fn('부서')||'</td>
                <td class="inputtd">
                  '); common.list_dept(nvl(rec.companycd, p_companycd), rec.teamcd, 'select', null, 'p_teamcd', 'b', v_disabled_select); 
--                  common.list_dept(v_comppp, rec.teamcd, 'select', null, 'p_teamcd', 'b', v_disabled_select); 
                  htp.p(' 
                </td>
              </tr>
