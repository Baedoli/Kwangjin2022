--------------------------------------------------------
--  DDL for Package Body KSCM2204
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PREMIER"."KSCM2204" 
IS
  PROCEDURE control
  (
      ...
      <title>'||common.title||'</title>
        ...
        <script type="text/javascript">
          ...
          function doInitGrid(){
            ...
            x[i]=["'||get_fn('사업구분')||'", "80","center","ro","str","true"];i++;
            x[i]=["'||get_fn('판매품번')||'", "120","left","ro","str","true"];i++;
            x[i]=["'||get_fn('자재품번')||'", "120","left","ro","str","true"];i++;
            x[i]=["'||get_fn('원자재품번')||'", "120","left","ro","str","true"];i++;
  ...
  IS
    cursor cur is
      select '9BUB/YC-K_KJM' as kindcd
            ,'1' as gubun
            ,'4270-2335' as p_partno
            ,'REG ASSY-PR LH' as ja_partno
            ,'3411' as one_partno
            ,1111 as m01
            ,1112 as m02
            ,1113 as m03
...
--      v_link := '^javascript:link_(\"'||rec(i).companycd||'\", \"'||rec(i).vendcd||'\", \"'||rec(i).partno||'\", \"'||rec(i).sno||'\")^_self';
      ...
      { id:' ||i|| ',
        data:[
          ...
          {value:"'||rec(i).kindcd||'"},
          {value:"'||rec(i).gubun||'"},
          {value:"'||rec(i).p_partno||'"},
          {value:"'||rec(i).ja_partno||'"},
          {value:"'||rec(i).one_partno||'"},
          {value:"'||rec(i).m01||'"},
          {value:"'||rec(i).m02||'"},
          {value:"'||rec(i).m03||'"},
          {value:"'||rec(i).m04||'"},
