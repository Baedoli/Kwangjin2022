--------------------------------------------------------
--  DDL for Package Body KSCM5117
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PREMIER"."KSCM5117" 
IS
  PROCEDURE control
  (
  ...
  <head>
    ...
    <script>
      ...
      function doCrrec() {  
        var url = "'||v_pkg||'.crrec?p_companycd='||p_companycd||'&p_vendcd='||p_cli_code||'";  
        newWnd2 = window.open(url,"'||v_pkg||'window","top=100, left=100, status=yes, resizable=no, scrollbars=yes, width=780, height=190");  
        newWnd2.focus();
      } 
      ...
      <title>'||common.title||'</title>
        ...
        <script type="text/javascript">
          ...
          function doInitGrid(){
            ...
            x[i]=["'||get_fn('그룹코드')||'", "120","center","link","str","true"];i++;
            x[i]=["'||get_fn('그룹명')||'", "*","center","link","str","true"];i++;
            x[i]=["'||get_fn('순번')||'", "250","center","link","str","true"];i++;
          ...
          function link_(obj1, obj2, obj3){
            url ="'||v_pkg||'.crrec?p_companycd="+obj1+"&p_vendcd="+obj2+"&p_groupcd="+obj3;
            newWnd2 = window.open(url,"'||v_pkg||'window","top=100, left=100, status=yes, resizable=no, scrollbars=yes, width=780, height=190");  
            newWnd2.focus();
          }
  ...
  BEGIN
    ...
    for i in 1 .. v_number loop
      v_link := '^javascript:link_(\"'||rec(i).companycd||'\", \"'||rec(i).vendcd||'\", \"'||rec(i).groupcd||'\")^_self';
      htp .p('
      { id:' ||i|| ',
        data:[
          {value:"'||rec(i).groupcd||v_link||'"},
          {value:"'||rec(i).descr||v_link||'"},
          {value:"'||rec(i).list_order||v_link||'"}
        ]
      } ');
  ...
  <body>                
    <form id="form1" name="form1" action="'||v_pkg||'.process" method="post" >
      ...
      <input type="hidden" id="p_companycd" name="p_companycd" value="'||p_companycd||'" />
      <input type="hidden" id="p_vendcd" name="p_vendcd" value="'||p_vendcd||'" />
      <input type="hidden" id="p_groupcd" name="p_groupcd" value="'||rec.groupcd||'" />
        ...
        <div>
          <table width="100%" border="0" cellspacing="0" cellpadding="4">
            <tbody>
              ...
              <tr>
                <td class="titletd">'||get_fn('그룹명')||'</td>
                <td class="inputtd">
                  <input type="text" id="p_descr" name="p_descr" value="'||rec.descr||'" /> 
                </td>
                <td class="titletd">'||get_fn('순번')||'</td>
                <td class="inputtd">
                  <input type="text" id="p_list_order" name="p_list_order" value="'||rec.list_order||'" / > 
                </td>
              </tr>
  ...
  IS
    ...
    cursor cur_max is
      select  nvl(max(groupcd), 0)+1
      from    mm4030t
      where   companycd = p_companycd
      and     vendcd = p_vendcd
      ;
    
    max_groupcd  number:= 0; 

    v_groupcd varchar2(20);

  BEGIN
    
    
    open cur_max;   
    fetch cur_max into max_groupcd;   
    close cur_max;
    
    if p_groupcd is null then
        v_groupcd := max_groupcd ;
    else
        v_groupcd := p_groupcd ;
    end if;
    ...
    if p_dml_gubun = '1' then
      ...
      begin
        ...
        insert into mm4030t 
        ( companycd, vendcd, groupcd, descr, list_order, login_id, login_ymd )
        values  
        ( p_companycd, p_vendcd, v_groupcd, p_descr, p_list_order, v_id, sysdate );
        EXCEPTION   
      ...
      htp.p('
        <script type="text/javascript">
          ...
          location.href = "'||v_pkg||'.crrec?p_companycd='||p_companycd||'&p_vendcd='||p_vendcd||'&p_groupcd='||v_groupcd||'";
        </script>
        ');
