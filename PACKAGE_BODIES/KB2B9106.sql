--------------------------------------------------------
--  DDL for Package Body KB2B9106
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PREMIER"."KB2B9106" 
IS
  PROCEDURE control
  (
  ...
  <div id="top-header" style="padding-bottom: 2px;background-color: #ffffff;">
    ...
    <div id="c00">
      <div id="c01">
        ...
        <div class="cBox">
          <div class="cTitle"><label for="p_companycd2">'||get_fn('수주사업장')||'</label></div>
          <div class="cInput">
            <select name="p_companycd2" id="p_companycd2">
            '); common.list_company(p_companycd2, v_lang); htp.p('
            </select>
          </div>
        </div> 
  ...
  IS
    cursor cur is
      select '20221011' as gubun from dual
      union
      select '20220912' as gubun from dual
      union
      select '20220720' as gubun from dual
      ;
...
--      v_link := '^javascript:link_(\"'||rec(i).companycd||'\", \"'||rec(i).vendcd||'\", \"'||rec(i).partno||'\", \"'||rec(i).sno||'\")^_self';
      ...
      <title>'||common.title||'</title>
        ...
        <script type="text/javascript">
          ...
          function doInitGrid(){
            ...
            x[i]=["'||get_fn('등록시간')||'", "100","left","ro","str","true"];i++;
  ...
  IS
    cursor cur is
      select 'busan' as place
            ,'2011/12/25' as require_date
            ,'1' as seq
            ,'k9' as kindcd
            ,'tank' as partno
            ,'501542' as qty
