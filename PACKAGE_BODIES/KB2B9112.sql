--------------------------------------------------------
--  DDL for Package Body KB2B9112
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PREMIER"."KB2B9112" 
IS
  PROCEDURE control
  (
      ...
      <title>'||common.title||'</title>
        ...
        <script type="text/javascript">
          ...
          function doInitGrid(){
            ...
            x[i]=["'||get_fn('납기요구일')||'", "*","center","ro","str","true"];i++;
  ...
  IS
    cursor cur is
      select '20210911' as gubun from dual
      union
      select '20211112' as gubun from dual
      union
      select '20220310' as gubun from dual
      ;
...
--      v_link := '^javascript:link_(\"'||rec(i).companycd||'\", \"'||rec(i).vendcd||'\", \"'||rec(i).partno||'\", \"'||rec(i).sno||'\")^_self';
      ...
      <title>'||common.title||'</title>
        ...
        <script type="text/javascript">
          ...
          function doInitGrid(){
            ...
            x[i]=["NO", "#rspan", "40","center","ro","int","true"];i++;
            x[i]=["'||get_fn('수주처')||'", "#rspan", "60","center","ro","str","true"];i++;
            x[i]=["'||get_fn('구분')||'", "#rspan", "60","center","ro","str","true"];i++;
            x[i]=["'||get_fn('품번')||'", "#rspan", "100","left","ro","str","true"];i++;
            x[i]=["'||get_fn('품명')||'", "#rspan", "180","left","ro","str","true"];i++;
            x[i]=["'||get_fn('차종')||'","#rspan",  "100","left","ro","str","true"];i++;
            x[i]=["'||get_fn('발주수량')||'", "#rspan", "60","left","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            x[i]=["'||get_fn('입고현황 ( 기준일자: )')||'", "'||get_fn('<-')||'", "60","left","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            x[i]=["#cspan","'||get_fn('')||'", "60","left","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            x[i]=["#cspan","'||get_fn('')||'", "60","left","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            x[i]=["#cspan","'||get_fn('')||'", "60","left","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            x[i]=["#cspan","'||get_fn('')||'", "60","left","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            x[i]=["#cspan","'||get_fn('')||'", "60","left","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            x[i]=["#cspan","'||get_fn('->')||'", "60","left","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            x[i]=["'||get_fn('합계')||'", "#rspan", "100","left","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            x[i]=["'||get_fn('미납')||'", "#rspan", "100","left","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
  ...
  IS
    cursor cur is
      select 'hwasin' as suju
            ,'내부' as gubun
            ,'12512-44121' as partno
            ,'benz' as partnm
            ,'mercedes' as kindcd
            ,'201542' as qty
            ,'302130' as inqty
...
--      v_link := '^javascript:link_(\"'||rec(i).companycd||'\", \"'||rec(i).vendcd||'\", \"'||rec(i).partno||'\", \"'||rec(i).sno||'\")^_self';
      ...
      { id:' ||i|| ',
        data:[
          ...
          {value:"'||rec(i).inqty_5||'"},
          {value:"'||rec(i).inqty_6||'"},
          {value:"'||rec(i).sum_data||'"},
          {value:"'||rec(i).cancle_data||'"}
        ]
      } ');
