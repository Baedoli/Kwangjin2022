--------------------------------------------------------
--  DDL for Package Body KSCM6106
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PREMIER"."KSCM6106" 
IS
  PROCEDURE control
  (
      ...
      <title>'||common.title||'</title>
        ...
        <script type="text/javascript">
          ...
          function doInitGrid(){
            ...
            x[i]=["'||get_fn('계획수량')||'", "100","right","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            x[i]=["'||get_fn('실적수량')||'", "100","right","ron","int","true"];mygrid.setNumberFormat("0,000", i);i++;
            x[i]=["'||get_fn('달성율(%)')||'", "100","right","ro","str","true"];i++;
